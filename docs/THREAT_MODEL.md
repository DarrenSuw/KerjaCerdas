...
- **Cross-tenant employer resource ownership:** every mutating job/candidate endpoint in `employer.py` (`PATCH`/`DELETE /employer/jobs/{id}`, `POST /employer/jobs/{id}/candidates`) is routed through a single `_require_owned_job()` helper that asserts `job.employer_id == caller's employer.id` before touching the row (403/404 otherwise). Consolidating the check in one helper is deliberate: hand-repeating an ownership check per endpoint is how one eventually gets added without it.
...
- **Skill quizzes are not cheat-proof, and the product says so.** Mitigations are layered: random questions from a bank of 30 per skill, per-attempt shuffled options, a server-side deadline, a retake cooldown, answers never returned, rate limiting on `/quiz/*`, and interview questions that ask the candidate to explain their own answer. HR confirmation is the final gate, and a candidate who cannot explain a passed quiz simply never reaches proof level 1.0. Guess-through on a 4-option, 4-of-5 quiz is **1.56%**, which is part of why a quiz badge is worth 0.85 and not 1.00.
- **Memorised retakes cannot buy a badge.** A retake never redraws the previous attempt's questions. Where a bank is still too thin to honour that, the quiz is served — locking a candidate out over our unfinished bank would be worse — but the attempt is marked `proof_eligible = false` and writes no evidence. The alternative considered and rejected was reporting the overlap and awarding the badge anyway: the harm is the badge, not the invisibility.
- **Paid question generation is a spend an attacker controls, so it is budgeted twice.** A skill with no bank costs ~Rp110 per batch to create, so generation requires the skill to already be on the requester's profile, is throttled per skill (15 min cold, 6 h for top-ups), **and** is capped per user (3/day). The per-skill throttle alone bounds nothing, because adding skills to a profile is free and unlimited — fifty invented skills would buy fifty generations.
- **Report brigading cannot remove a posting.** Reports are weighted, not counted: an unverified or brand-new account carries zero automatic weight, and a reporter whose accusations keep being dismissed stops counting entirely. Reaching the threshold marks a posting `flagged`, which **stays publicly visible**; only a confirmed violation of a **hard** rule — checked by the AI reviewer against the specific rule the reporter cited — hides it, and an admin can always overturn that. A soft-rule finding or an uncertain verdict queues a human instead. The failure mode being designed against is an uncertain model removing a real employer's advert, which would cost the side of the market that is hardest to win back.
