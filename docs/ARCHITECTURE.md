...
| Applicant review | Employer opens the Pelamar tab | `ApplicantList.jsx` → `GET /api/v1/employer/applications` | Applicants ranked by live proof-weighted score, proof badges, pipeline status; Spark ranks the first 20 |
...
| Hybrid ranking | Vector similarity + structured profile/job fields + **skill proof levels** | `SemanticMatcher` reranking (`matcher.py`) with proof weights from `matching/evidence.py` | `0.45` cosine + `0.30` proof-weighted skills (claimed 0.30 / quiz 0.85 / HR-confirmed 1.00; required skills 80%, nice-to-have 20%) + `0.15` experience + `0.10` education vs `education_min`. The old flat `0.05` recency term was removed — it was identical for every candidate | Ranked `MatchResult` list with band, per-factor breakdown and per-skill proof status |
| Skill quiz grading & generation | 5 random questions, per-attempt shuffled options, server-side deadline | Answer-key comparison for grading (**no AI call**, attempt costs Rp0). If skill is missing, AI generates questions once via Gemini | Deterministic grading + AI generation fallback | Pass (≥4/5) sets the skill's proof to `quiz` for 180 days |
