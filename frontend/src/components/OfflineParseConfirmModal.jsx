/**
 * OfflineParseConfirmModal — shown when parse_cv falls back to offline/demo
 * data (parsed_offline: true). The user must explicitly confirm before the
 * result is committed to their profile.
 *
 * Props:
 *   preview  { full_name, headline, skills: string[] }
 *   onConfirm()  — called when user clicks "Tetap Simpan"
 *   onCancel()   — called when user clicks "Batalkan" or backdrop
 */
import { KC, DesignStyles } from './_design'
import { AlertTriangle, WifiOff } from 'lucide-react'

export default function OfflineParseConfirmModal({ preview, onConfirm, onCancel }) {
    if (!preview) return null

    const skills = (preview.skills || []).slice(0, 8)
            ...
            >
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                    ...
                    <div>
                        ...
                        <p style={{
                            ...
                            Gemini API tidak tersedia saat ini. Data di bawah berasal dari mode
                            fallback dan mungkin tidak akurat atau merupakan data demo.
                        </p>
                    </div>
                </div>
                ...
                <div style={{
                    ...
                    <span style={{
                        ...
                        Menyimpan ini akan menimpa profil yang sudah ada.
                    </span>
                </div>
                ...
                <div style={{
                    ...
                    <PreviewRow label="Nama" value={preview.full_name || '—'} />
                    <PreviewRow label="Posisi / Headline" value={preview.headline || '—'} />

                    {skills.length > 0 && (
                        <div>
                            <p style={{ margin: '0 0 6px', font: '600 11.5px/1 "Plus Jakarta Sans", sans-serif', color: KC.mute }}>Skill terdeteksi</p>
...
function PreviewRow({ label, value }) {
    return (
        <div style={{ display: 'flex', gap: 10, alignItems: 'baseline' }}>
            <span style={{ font: '600 11.5px/1 "Plus Jakarta Sans", sans-serif', color: KC.mute, minWidth: 110 }}>{label}</span>
